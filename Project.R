...
applyRules=function(currentPortfolio,day,equity){
  ...
  if(length(currentPortfolioSignals)!=0){
  ...
  signals <- signals[!(signals$short_exit==1&signals$long_entry==0&signals$short_entry==0),]
  signals <- signals[!(signals$long_exit==1&signals$long_entry==0&signals$short_entry==0),]
  candidates<-subset(signals,signals$date==day&signals$long_entry==1)
  longs<-genTrades(candidates,3*equity/4)
  equity<-equity+longs$cashin-longs$cashout-longs$transcost
  candidates<-subset(signals,signals$date==day&signals$short_entry==1)
  candidates$prediction<-2-candidates$prediction
  shorts<-genTrades(candidates,equity)
  
  currentPortfolioSignals <-  currentPortfolioSignals[!( currentPortfolioSignals$short_entry==1&currentPortfolioSignals$long_exit==0&currentPortfolioSignals$short_exit==0),]
  currentPortfolioSignals <-  currentPortfolioSignals[!( currentPortfolioSignals$long_entry==1&currentPortfolioSignals$long_exit==0&currentPortfolioSignals$short_exit==0),]
  #Remove buy for already long open position
  currentPortfolioSignals <- currentPortfolioSignals[!(currentPortfolioSignals$short_exit == 1 & currentPortfolioSignals$position > 0), ]
  #Remove sell for already short open position
  currentPortfolioSignals <- currentPortfolioSignals[!(currentPortfolioSignals$long_exit == 1 & currentPortfolioSignals$position < 0), ]
...
# ********************************  RUN STRATEGY **********************************
cat("\014")
if(!file.exists('ProjectUniverse.rdata')){
universe<-getData()
}else{
...
if (proceed=="Y"){
  ...
  #browser()
  signals<-NULL  
  indicators <- indicators[!is.na(indicators$open),]
  indicators <- indicators[!is.na(indicators$doublelagged.rsi2),]
  indicators <- indicators[!is.na(indicators$lagged.rsi2),]
  indicators <- indicators[!is.na(indicators$trend.down),]
  indicators <- indicators[!is.na(indicators$trend.up),]
